    ...
    def _apply_hard_constraints(self):
        ...
        for c_id in self.classes:
            ...
            for s_id in required_subjects:
                ...
                if block_size > 1:
                    # If subject needs 2-hour blocks, enforce adjacency
                    # Simplified strategy: Penalize singletons or force starts
                    pass 
                    # (Full implementation of sliding window constraint omitted for brevity, 
                    # but essentially requires checking var[d][p] AND var[d][p+1])
