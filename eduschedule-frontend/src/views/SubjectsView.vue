...
<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      ...
      <button @click="showAddModal = true" class="bg-brand-primary text-white font-bold py-2 px-4 rounded-lg hover:bg-brand-secondary transition-colors">
        + Add Subject
      </button>
    </div>
      ...
      <table class="min-w-full">
        ...
        <tbody>
          <tr v-if="subjects.length === 0">
            <td colspan="2" class="py-8 text-center text-gray-500">No subjects found. Click "Add Subject" to create one.</td>
          </tr>
        ...
        <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-md">
          ...
          <form @submit.prevent="handleAddSubject">
            <div class="mb-4">
              ...
              <input v-model="newSubject.name" type="text" id="name" class="w-full p-2 border rounded-md focus:ring-2 focus:ring-brand-primary outline-none" required>
            </div>
            <div class="mb-6">
              <label class="block text-brand-gray mb-1" for="periods">Periods per Week</label>
              <input v-model.number="newSubject.periods_per_week" type="number" id="periods" class="w-full p-2 border rounded-md focus:ring-2 focus:ring-brand-primary outline-none" required min="1" max="40">
            </div>
            <div class="flex justify-end space-x-4">
              <button type="button" @click="showAddModal = false" class="bg-gray-200 px-4 py-2 rounded-lg hover:bg-gray-300 transition-colors">Cancel</button>
              <button type="submit" class="bg-brand-accent text-brand-dark font-bold px-4 py-2 rounded-lg hover:opacity-90 transition-opacity">Save Subject</button>
            </div>
          </form>
        </div>
      </div>
