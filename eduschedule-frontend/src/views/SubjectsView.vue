  ...
  <div>
    ...
    <div v-else class="bg-white rounded-lg shadow-md overflow-hidden">
      <table class="min-w-full">
        ...
        <tbody>
          <tr v-for="subject in subjects" :key="subject.id" class="border-t">
        ...
        <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-md">
          ...
          <form @submit.prevent="handleAddSubject">
            <div class="mb-4">
              ...
              <input v-model="newSubject.name" type="text" id="name" class="w-full p-2 border rounded-md" required>
            </div>
            <div class="mb-6">
              <label class="block text-brand-gray mb-1" for="periods">Periods per Week</label>
              <input v-model.number="newSubject.periods_per_week" type="number" id="periods" class="w-full p-2 border rounded-md" required>
            </div>
            <div class="flex justify-end space-x-4">
              <button type="button" @click="showAddModal = false" class="bg-gray-200 px-4 py-2 rounded-lg">Cancel</button>
              <button type="submit" class="bg-brand-accent text-brand-dark font-bold px-4 py-2 rounded-lg">Save Subject</button>
            </div>
          </form>
        </div>
      </div>
